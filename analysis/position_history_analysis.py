...
def main(targetYear=None, targetMonth=None):
    ...
    ft.clean_target_dir(setting.get_trade_open_dir())
    ...
    for record in record_list:
        ...
        if (record.kbn == 'open'):
            ft.create_target_dir(os.path.join(setting.get_trade_dir(), record.kbn))
            csv_file_full_path = os.path.join(setting.get_trade_dir(), record.kbn, file_name)
        else:
            ft.create_target_dir(os.path.join(setting.get_trade_dir(), record.year))
            ft.create_target_dir(os.path.join(setting.get_trade_dir(), record.year, record.month))
            csv_file_full_path = os.path.join(setting.get_trade_dir(), record.year, record.month, file_name)
