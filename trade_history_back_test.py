...
import os
import pandas as pd
from util import fileutil as ft, tradeutil
import numpy as np
...
EXIT_MAX_OPEN_DAYS=20
# Stop Loss
EXIT_stop_loss_rate=-0.05 # total 2.5%
# Stop Profit
EXIT_stop_profit_rate=0.05 # total 2.5%
...
parser.add_argument("--backtest_start_ymd", type=str, default=None,
                    help="Start date YMD(close) of backtest. e.g. 20191101")
parser.add_argument("--backtest_end_ymd", type=str, default=None,
                    help="End date YMD(close) of backtest. e.g. 20211101")
...
# Setup logger
LogHelper.setup(log_path='{0}/backtesting_{1}.log'.format(config.log_output_dir, runDateTime), log_level=logging.DEBUG)
...
def main():
    ...
    pre_process()
    history_data_file_full_path = output_current_trade_history_data()
    generate_trade_report(history_data_file_full_path)
...
def generate_trade_report(history_data_file_full_path):
    ...
    if config.backtest_start_ymd is not None:
        trade_history_data = trade_history_data[trade_history_data.close > config.backtest_start_ymd]

    if config.backtest_end_ymd is not None:
        trade_history_data = trade_history_data[trade_history_data.close < config.backtest_end_ymd]
    ...
    for index, row in trade_history_data.iterrows():
        ...
        if trade_data is None:
            _logger.error("No target data found. {0} - {1}...{2}".format(sellCode, buytCode, year + month))
            continue
...
def generate_result(search_data, sellCode, buyCode, history_row):
    ...
    try:
        ...
        _logger.error("No target data found(Open day is not correct). {0} - {1}...{2}".format(sellCode, buyCode, trade_start_date))
    ...
    for index, row in search_data.iterrows():
        ...
        elif count == len(search_data) - 1:
            _logger.info("There is not have more stock date for backtest. {0} - {1}...{2}".format(sellCode, buyCode,
                                                                            trade_start_date))
            trade_result = 'OVER MAX DAY_TEMP'
...
def getTargetTradeData(year, month, sellCode, buyCode):
    ...
    try:
        ...
        _logger.error("Error. {0} - {1}...{2}/{3}...{4}".format(sellCode, buyCode, year, month, targetFilePath))

    return trade_data

def output_current_trade_history_data():
    ...
    return history_output_csv_file_full_path
